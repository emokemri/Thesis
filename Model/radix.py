...
class RadixSystem(ObservableModel):
    ...
    def calculate_determinant_abs(self):
        ...
        return abs(np.linalg.det(self.matrix)).round().astype(int)
    ...
    def check_crs(self):
        ...
        if len(hash_values) == self.calculate_determinant_abs():
            return True
    ...
    def find_periodic_points(self) -> None:
        ...
        if len(self.lattice_points) > 2000:
            self.lattice_points.clear()
            return
    ...
    def is_radix(self):
        ''' Check if the given system is radix system '''
        if self.check_crs() and self.expansive_matrix():
            if self.dimension == 1:
    ...
    def is_GNS(self):
        ...
        if self.check_crs() and self.expansive_matrix() and self.determinant_check():
            ...
            if zeros_tuple in self.periodic_points and len(self.periodic_points) == 1:
                return True
            else:
                return False
